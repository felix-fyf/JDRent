...
namespace JDRent\Support\Security\Rsa;
...
class RSA
{
    ...
    */
    ...
    function _none_hash_pkcs1_v1_5_encode($m, $emLen) {
        $mLen = strlen($m);
        $ps = str_repeat(chr(-1), $emLen - $mLen - 3);
        $em = "\0\1$ps\0$m";
        return $em;
    }
    ...
    {
        if ($this->hashName == 'none') {
            return $this->_none_hash_pkcs1_v1_5_encode($m, $emLen);
        }
    ...
    function sign_with_md5_then_rsa($message, $md5Salt) {
        $clp = $message.$md5Salt;
        $digest = md5($clp);
        return $this->sign($digest);
    }

    function verify_with_md5_then_rsa($message, $md5Salt, $sign) {
        $clp = $message.$md5Salt;
        $digest = md5($clp);
        return $this->verify($digest, $sign);
    }
